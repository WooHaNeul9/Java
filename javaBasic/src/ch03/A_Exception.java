...
// 에러와의 차이
// 1. 에러가 발생하면 프로그램이 실행조차 되지 않지만 예외는 실행은 됨
// 2. 에러는 개발자가 직접 처리하기 힘든 경우가 대부분이지만, 예외는 개발자가 직접 처리할 수 있음
...
public class A_Exception {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// finally로 에외가 발생하든 안 하든 실행할 코드를 작성
			// 자원의 반환처리를 대부분 finally에서 작업함
			scanner.close();
