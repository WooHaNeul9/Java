package ch02;

// 다형성 : 하나의 표현이 여러가지 형태로 나타낼 수 있는 것

// 오버로드 : 같은 클래스 내에서 동일한 메서드의 이름으로 여러개의 메서드를 정의할 수 있도록 하는 것
// 오버라이드 : 자손 클래스에서 조상 클래스에서 정의된 메서드를 재정의 하는 것

class Human1 {
	String name;
	int age;
}

class Developer1 extends Human1 {
	String positon;
}

class FootballPlayer1 extends Human1 implements FootballPlay {
	int goal;

	@Override
	public void kick() { }
}

interface FootballPlay {
	public abstract void kick();	
} 

public class H_Polymorphism {

	public static void main(String[] args) {
		
		Human1 human1 = new Human1();
		
		Developer1 developer1 = new Developer1();
		FootballPlayer1 footballPlayer1 = new FootballPlayer1();
		
		// 업캐스팅 : 하위 클래스의 객체를 상위 클래스의 참조변수에 담을 수 있도록 변환하는 것
		// 			인터페이스 구현 클래스를 구현한 인터페이스의 타입 참조변수로 담을 수 있도록 하는 것
		Human1 human2 = new Developer1();
		Human1 human3 = new FootballPlayer1();
		
		FootballPlay football = new FootballPlayer1();
		
		// human2.position = "";
		
		// 다운캐스팅 : 업캐스팅된 객체를 다시 원래의 타입의 참조변수에 담을 수 있도록 변환하는 것
		// 업캐스팅했을 때 담은 참조변수를 하위 클래스에 그대로 담아서 가져옴
		Developer1 developer2 = (Developer1)human2;
		developer2.positon = "";
		
		FootballPlayer1 fp = new FootballPlayer1();
		fp.name = "홍길동";
		fp.age = 30;
		fp.goal = 55;
		
		System.out.println(fp); // 결과값 : ch02.FootballPlayer1@515f550a
		System.out.println(fp.name); // 결과값 : 홍길동
		System.out.println(fp.goal); // 결과값 : 55
		
		Human1 hm = fp;
		System.out.println(hm); // 결과값 : ch02.FootballPlayer1@515f550a
		System.out.println(hm.name); // 결과값 : 
//		System.out.println(hm.goal); => 업캐스팅을 했기 때문에 fp에는 Human1의 정보만 들어가 있음 
//		System.out.println(hm.kick); => 업캐스팅을 했기 때문에 fp에는 Human1의 정보만 들어가 있음
		
		FootballPlayer1 fpp = (FootballPlayer1)hm;
		System.out.println(fpp); // 결과값 : ch02.FootballPlayer1@515f550a
		System.out.println(fpp.name); // 결과값 : 홍길동
		System.out.println(fpp.goal); // 결과값 : 55

//---------------------------------------------------------------------------------------------//			
//		Human1 hmm = new Human1();
//		Developer1 dev = (Developer1)hmm;
//		
//		dev.positon = "포지션";
//		System.out.println(dev.positon); // 결과값 : Exception 발생
		
//		=> Exception 발생 : dev가 인식할 수 없는 것을 억지로 인식하게 선언하였기 때문
//---------------------------------------------------------------------------------------------//
//		=> 밑의 코드처럼 작성하면 hmm에 Developer1 변수들이 담겨서 다운캐스팅 했을 시 '포지션'이라는 결과가 나옴
		
//		Human1 hmm = new Developer1();
//		Developer1 dev = (Developer1)hmm;
//		
//		dev.positon = "포지션";
//		System.out.println(dev.positon); // 결과값 : 포지션
//---------------------------------------------------------------------------------------------//			
		
		// Object 클래스의 참조변수는 어떠한 클래스의 인스턴스도 받을 수 있음
		Object object1 = new Human1();
		Object object2 = new Developer1();
		Object object3 = new FootballPlayer1();
		
	}
}