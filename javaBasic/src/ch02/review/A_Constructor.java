...
class ConstructorClass {
	...
	// 셍성자 : 클래스의 인스턴스를 생성할 때 호출되는 메서드
	// 생성시에 실행할 코드를 구현한 메서드
	// 일반적으로 인스턴스 변수 초기화 작업에 사용
	// 생성자를 개발자가 직접 작성하지 않으면 자바가 기본 생성자를 자동으로 생성함	
	...
	ConstructorClass() {
		// 아래와 같은 상황을 '의존성을 내부에서 주입한다'라고 표현함
	...
	ConstructorClass(int instanceVariable1){
		...
		// 글로벌로 위치에 있는 변수 혹은 메서드를 지칭할 때 사용
		instanceVariable1 = instanceVariable1; // ( X )
		this.instanceVariable1 = instanceVariable1; // ( O )
	}
