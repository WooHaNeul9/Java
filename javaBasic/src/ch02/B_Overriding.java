...
//	}
	...
	// 'void instanceMathod()' 메서드를 이미 가지고 있기 때문에 아래 메서드는 'void instanceMethod()'를 오버로드 한 것
...
class SubClass2 extends SuperClass1 {
	...
	@Override // 컴파일러에게 아래 메서드가 오버라이드 메서드라는 것을 알려주는 것 => 가독성 향상, 개발자의 실수 방지 
	void instanceMethod() {
		// super 키워드 : 자손(하위) 클래스에서 조상(상위) 클래스의 필드 혹은 메서드에 접근할 때 사용
...
public class B_Overriding {
	...
	public static void main(String[] args) {
		...
		instance1.instanceMethod(); // 결과값 : 조상 클래스의 메서드입니다.
		instance2.instanceMethod(); // 결과값 : 조상 클래스의 메서드입니다.
		instance3.instanceMethod(); // 결과값 : super 키워드 사용 전 => SubClass2의 인스턴스 메서드입니다. // super 키워드 사용 후 => 조상 클래스의 메서드입니다. SubClass2의 인스턴스 메서드입니다.
		
		System.out.println(instance1.superInstanceVariable); // 결과값 : 0
		System.out.println(instance2.superInstanceVariable); // 결과값 : 10
		System.out.println(instance3.superInstanceVariable); // 결과값 : 50
